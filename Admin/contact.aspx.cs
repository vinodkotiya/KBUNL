...
public partial class Admin_news_post : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            System.Web.UI.HtmlControls.HtmlControl menuli = (System.Web.UI.HtmlControls.HtmlControl)this.Master.FindControl("menu_contacts");
        ...
        try
        {
            if (btnSave.Text == "Save")
            {
                if (txtOfficeName.Text.Trim() == "")
                    displayMessage("Please enter Office Name", "error");
                ...
                {
                    string[] parameter = { "@Flag", "@OfficeName", "@Address", "@ContactNo", "@EmailID", "@Website" };
                    string[] value = { "Insert", txtOfficeName.Text.Trim(), txtAddress.Text.Trim(), txtContactNo.Text.Trim(), txtEmailID.Text.Trim(), txtWebsite.Text.Trim() };
                    DB_Status dbs = dba.sp_populateDataSet("SP_Contacts", 6, parameter, value);
                    string result = "";
            ...
            else if (btnSave.Text == "Update")
            {
                ...
                {
                    string[] parameter = { "@Flag", "@RID", "@OfficeName", "@Address", "@ContactNo", "@EmailID", "@Website" };
                    string[] value = { "Update", hfRID.Value, txtOfficeName.Text.Trim(), txtAddress.Text.Trim(), txtContactNo.Text.Trim(), txtEmailID.Text.Trim(), txtWebsite.Text.Trim() };
                    DB_Status dbs = dba.sp_populateDataSet("SP_Contacts", 7, parameter, value);
                    string result = "";
                    if (dbs.OperationStatus.ToString() == "Success")
                    {
                        DataSet ds = dbs.ResultDataSet;
    ...
    protected void FillRecords()
    {
        try
        {
            ...
            string[] value = { "View" };
            DB_Status dbs = dba.sp_populateDataSet("SP_Contacts", 1, parameter, value);
            if (dbs.OperationStatus.ToString() == "Success")
            {
                DataSet ds = dbs.ResultDataSet;
                if (ds.Tables.Count > 0)
    ...
    protected void Update_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            DB_Status dbs = dba.sp_populateDataSet("SP_Contacts", 2, parameter, value);
            bool flag = false;
