...
public partial class WCCommon_WCTopBar : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            divProjectInformation.InnerHtml = ATSPLLib.GetProjectInformation("kbunl", "Hindi");
        ...
        try
        {
            ...
            if (dbs.OperationStatus.ToString() == "Success")
            {
                ...
                if (ds.Tables.Count > 0)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        Page.Title = Convert.ToString(dt.Rows[0]["ProjectNameHindi"]);
                    }
                }
            }
        }
                    ...
                    {                        
                        ...
                        for (int i = 0; i < dtMenu.Rows.Count; i++)
                        {
                            if (Convert.ToString(dtMenu.Rows[i]["MenuLevel"]) == "Level1")
                            {
                                ...
                                ltrMenu.Text = ltrMenu.Text + "<li><a href='" + Convert.ToString(dtMenu.Rows[i]["LinkURL"]) + "' " + target + ">" + Convert.ToString(dtMenu.Rows[i]["MenuTitleH"]) + "</a>";
    ...
    protected void lnkbtnMedium_Click(object sender, EventArgs e)
    {
        ...
        Response.Redirect(Request.RawUrl.Replace("/Hindi", "/English"));
    }
