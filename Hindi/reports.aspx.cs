...
public partial class reports : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            BindDepartment();
            LoadReports();
            lblReportTitle.Text = "रिपोर्ट शीर्षक";
            lblDate.Text = "";
        }
    }
                ...
                if (ds.Tables.Count > 0)
                {
                    ...
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            if (ReportGroup == "True")
                            {
                                ltrReports.Text = ltrReports.Text + "<td class='title'>" + Convert.ToString(dt.Rows[i]["ReportTitleHindi"]) + "</td>";
                            ...
                            {
                                ltrReports.Text = ltrReports.Text + "<td class='title'><a target='_blank' href='../" + Convert.ToString(dt.Rows[i]["AttachmentUrl"]) + "'>" + Convert.ToString(dt.Rows[i]["ReportTitleHindi"]) + "</a></td>";
