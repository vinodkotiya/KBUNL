...
public partial class directory : System.Web.UI.Page
{
    ...
    protected void BindDropdownlist()
    {
        ...
        if (DBS.OperationStatus.ToString() == "Success")
        {
            ...
            ddlBloodGroup.Items.Insert(0, "All");
            ddlBloodGroup.Items[0].Value = "0";

            //Table index 5 for Blood Group
            ddlDepartmentGroup.DataSource = ds.Tables[5];
            ddlDepartmentGroup.DataTextField = "DeptGroup";
            ddlDepartmentGroup.DataValueField = "DeptGroupID";
            ddlDepartmentGroup.DataBind();
            ddlDepartmentGroup.Items.Insert(0, "All");
            ddlDepartmentGroup.Items[0].Value = "0";
        }
    }
    ...
    StringBuilder SearchCondition()
    {
        ...
        if (ddlDepartmentGroup.SelectedValue == "0" && ddlDepartment.SelectedValue != "0")
        {
            sb.Append(" AND E.DeptID=" + Convert.ToInt32(ddlDepartment.SelectedValue));
        }
        else if (ddlDepartmentGroup.SelectedValue != "0" && ddlDepartment.SelectedValue == "0")
        {
            string qry = @" AND E.DeptID in (SELECT DeptID
                            FROM tbl_Department
                            WHERE IsActive=1 and IsDeleted=0
                            AND ReportingTo_DeptGroupID in
                            (
	                            select DeptGroupID
	                            from tbl_DepartmentGroup 
	                            where 
	                            DeptGroupID=" + ddlDepartmentGroup.SelectedValue + @" or 
	                            ReportingTo_DeptGroupID=" + ddlDepartmentGroup.SelectedValue + @"
                            )
                            OR Department=(Select DeptGroup from tbl_DepartmentGroup where DeptGroupID=" + ddlDepartmentGroup.SelectedValue + @")) ";
    ...
    protected void btnExport_Click(object sender, ImageClickEventArgs e)
    {
        try
        {
            ...
            string[] parameter = { };
            string[] value = { };
            DB_Status dbs = dba.sp_populateDataSet("SP_Export_Employee_Directory1", 0, parameter, value);
            if (dbs.OperationStatus.ToString() == "Success")
            {
                DataSet ds = dbs.ResultDataSet;
                if (ds.Tables.Count > 0)
            ...
            if (totalrecords > 0)
            {
                ...
                ws.Cells["N1"].Value = "DesignationH(H)";
                 ws.Cells["O1"].Value = "QuarterType";
                  ws.Cells["P1"].Value = "QuarterNo";
                     ws.Cells["Q1"].Value = "BloodGroupID";
                for (int i = 1; i <= totalrecords; i++)
                {
                    ...
                    ws.Cells[i + 1, 14].Value = tbl.Rows[i - 1]["DesignationH"].ToString();
                     ws.Cells[i + 1, 15].Value = tbl.Rows[i - 1]["QuarterType"].ToString();
                      ws.Cells[i + 1, 16].Value = tbl.Rows[i - 1]["QuarterNo"].ToString();
                        ws.Cells[i + 1, 17].Value = tbl.Rows[i - 1]["BloodGroupID"].ToString();
                }
    ...
    protected void ddlDepartmentGroup_SelectedIndexChanged(object sender, EventArgs e)
    {
        LoadDept_forDeptGroup();
        LoadEmployeeDirectory();
    }
    protected void LoadDept_forDeptGroup()
    {
        string[] param = { "@Flag", "@ReportingTo_DeptGroupID" };
        string[] values = { "LoadDept_forDeptGroup", ddlDepartmentGroup.SelectedValue };
        DB_Status DBS = dba.sp_populateDataSet("SP_Employees_Admin", 2, param, values);
        if (DBS.OperationStatus.ToString() == "Success")
        {
            DataSet ds = DBS.ResultDataSet;
            //Table index 0 for Department
