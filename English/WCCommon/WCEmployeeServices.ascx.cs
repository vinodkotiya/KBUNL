            ...
            if (dtEmpServices != null)
            {
                for (int i = 0; i < dtEmpServices.Rows.Count; i++)
                {
                    if (dtEmpServices.Rows[i]["URLOpenIn"].ToString() == "Open In New Page")
                        HTMLCode += "<a href='" + dtEmpServices.Rows[i]["ServiceLink"].ToString() + "' target='_blank'><span class='imgbox'><img src='../" + dtEmpServices.Rows[i]["Icon"].ToString() + "' alt=''></span><span class='title'>" + dtEmpServices.Rows[i]["ServiceTitleEnglish"].ToString() + "</span></a>";
                    else
                        HTMLCode += "<a href='" + dtEmpServices.Rows[i]["ServiceLink"].ToString() + "'><span class='imgbox'><img src='../" + dtEmpServices.Rows[i]["Icon"].ToString() + "' alt=''></span><span class='title'>" + dtEmpServices.Rows[i]["ServiceTitleEnglish"].ToString() + "</span></a>";
                }
            }
